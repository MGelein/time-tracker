...
const ignoreItems = "pauze break koffie coffee tea lunch dinner".split(" ");
...
function generateCalculation() {
  ...
  for (let i = 1; i < projects.length; i++) {
    ...
    if (!ignoreItems.includes(project.name.toLowerCase())) totalTime += timeSpent;
  ...
  console.log(`\n${bold(blue("Total:\t"))}${bold(purple(totalTime + "h"))}`);
}
...
function logNewEntry({ name, time }) {
  console.log(`Worked on ${blue(name)} until ${purple(time)}\n`);
  console.log(`execute ${blue("register")} ${purple("undo")} to remove this entry.`);
}
...
function registerProject(name, timeString) {
  ...
  if (existingProjects.length < 1)
    showErrorAndAbort(`No existing projects, please start the day first by running: ${blue("register start")}`);
  ...
  if (timePassed <= 0)
    showErrorAndAbort(
      `Registered time ${newEntry.time} is not after previously registered time ${previousProject.time}`
    );
